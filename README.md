...
### 主要更新
- 开发模式支持“页面配置优先”：在“系统设置 -> 开发模式”维护仓库、分支、自动拉取、代理等参数。
- 新增“重启并应用”：点击后写入运行配置并触发重启，减少手动改 compose 与重复重启操作。
- 新增最近应用状态：支持 success/failed/skipped/requested 状态展示与自动刷新，便于排查拉取/同步问题。
- 安全优化：Git token 传递改为 Authorization header，降低凭据暴露风险。
- 工程优化：新增 `instance/*` 忽略规则，避免本地数据库与运行标记误提交。
