...
### 主要更新
- 设置页开发模式交互完善：支持“保存开发模式设置”“保存并重启应用”，并在重启前先保存当前表单。
- 开发模式仓库地址兜底：页面为空时默认使用项目仓库地址，降低首次配置成本。
- 映射/定时/诊断/设置页面术语统一：用户侧将“回填”统一为“自动关联”，减少理解歧义。
- 日志筛选类型可读性提升：类型下拉与列表显示使用中文标签，同时保持后端原始键值不变。
- 映射筛选说明优化：明确“待判定”含义与自动关联窗口关系，降低误操作风险。
