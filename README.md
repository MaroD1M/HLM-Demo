...
### 主要更新
- 统一多个核心页面的说明区交互：支持“页面说明”折叠展开，默认收起，提升主内容可见性。
- 日志与执行历史页面排版优化：顶部操作区紧凑化、筛选区重排、执行历史支持分页。
- 文件映射与缓存管理页面排版优化：视图切换、筛选区与主内容区统一收口，提升小屏可读性。
- 分页体验升级：日志页与映射/缓存页新增每页条数选项（10/20/50/100），默认 20。
- 硬链接任务新建/编辑页优化：入库行为并入基础设置，文案统一为“创建同名目录”，返回入口更醒目。
